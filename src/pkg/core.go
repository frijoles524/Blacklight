...
func Download(p *Package) error {
	...
	if err := InstallRequirements(fmt.Sprintf("%s-%s/requirements.txt", p.Name, p.Version)); err != nil {
		fmt.Println("Package was downloaded, but dependencies were not installed correctly. To fix this manually edit the cache and remove the installation folder of the software in question.")
		panic(err)
	}
